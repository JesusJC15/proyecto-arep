...
Diseñar la arquitectura de una plataforma modular de triaje médico asistido por IA que combine aplicaciones web, backend orientado a servicios, almacenamiento operativo, un índice semántico reproducible y un pipeline RAG orientado a recomendaciones clínicas trazables.
...
\midrule
\texttt{01} Contexto & Contexto & Mostrar actores, plataforma, LLM, base documental y adaptador FHIR futuro. \\
\texttt{02} Contenedores & Contenedores & Delimitar frontends, API, servicios de triage/RAG y persistencia. \\
\texttt{03} Backend & Componentes & Detallar modulos internos del backend del MVP. \\
\texttt{04} Triage & Secuencia & Describir el flujo principal desde sintomas hasta recomendacion. \\
\texttt{05} Escalamiento & Secuencia & Describir el flujo de escalamiento hacia el profesional. \\
\texttt{06} AWS & Despliegue & Mapear la solucion objetivo sobre servicios de AWS. \\
\texttt{07} Datos & Modelo de datos & Representar entidades y relaciones del MVP. \\
\bottomrule
\end{tabularx}
\end{table}
...
\paperdiagram
  {01-c4-context.png}
  {Vista de contexto de la plataforma de triaje, sus actores primarios y dependencias externas del ecosistema propuesto.}
  {fig:c4_context}

\subsection{Contenedores principales}

La arquitectura general se compone de un frontend para paciente, un frontend profesional, un backend FastAPI, un módulo de intake clínico, un módulo de triage, un módulo RAG, un módulo de trazabilidad, persistencia operativa y un índice semántico reproducible. La Figura \ref{fig:c4_container} ayuda a observar que dichos contenedores no operan como piezas aisladas, sino como una cadena de valor cuyo producto final es una recomendación fundamentada y, cuando aplica, un caso listo para revisión profesional.

\paperdiagram
  {02-c4-container.png}
  {Vista de contenedores del MVP y de sus principales dependencias de persistencia, retrieval e interfaces de usuario.}
  {fig:c4_container}

\subsection{Arquitectura cloud objetivo}

La proyección en nube privilegia la separación entre canales de atención, servicios de negocio y capacidades de IA. La solución objetivo considera hosting web con S3 y CloudFront, una puerta de entrada API, un repositorio de imágenes en Amazon ECR, ejecución de servicios contenedorizados en ECS Fargate, almacenamiento relacional en Amazon RDS, soporte vectorial mediante OpenSearch o extensiones vectoriales sobre PostgreSQL y un servicio de modelos equivalente a Amazon Bedrock para embeddings y generación.
...
\paperdiagram
  {06-aws-deployment.png}
  {Vista de despliegue objetivo sobre AWS, alineada con la evolución futura del MVP hacia una arquitectura gestionada y observable.}
  {fig:aws_deployment}
...
Persistencia vectorial & OpenSearch o PostgreSQL vectorial gestionado & Índice local reproducible con ruta futura a \texttt{pgvector} \\
...
Operacion & Despliegue distribuido con monitoreo y logs centralizados & Ejecucion local reproducible con Docker Compose y observabilidad minima \\
\bottomrule
\end{tabularx}
\end{table}
...
Internamente, el backend se separa en auth, consultations, triage, rag y professional\_cases. Esta organización hace visible una primera decisión de modularidad: autenticación, reglas de severidad, recuperación documental y lectura de casos escalados no comparten una única pieza monolítica, sino responsabilidades diferenciadas que luego podrán migrar hacia una arquitectura más distribuida. La Figura \ref{fig:backend_components} muestra con mayor precisión este despiece interno del backend.

\paperdiagram
  {03-backend-components.png}
  {Descomposición de componentes del backend del MVP, con separación entre autenticación, consultas, triage, retrieval y gestión profesional.}
  {fig:backend_components}

\subsection{Implementación observable en el repositorio}

El repositorio ya materializa elementos concretos del MVP. En backend, \path{app/main.py} expone la API y registra rutas para autenticación, consultas y casos profesionales; \path{consultations.py} implementa la creación de consultas y la ejecución del triage; \path{triage_engine.py} calcula severidad a partir de intensidad y palabras clave de alerta; y \path{rag_service.py} carga el corpus curado desde \path{knowledge-base}, construye un índice reproducible y recupera hasta tres fuentes relevantes para componer la recomendación. En frontend, el portal del paciente representa la captura estructurada de síntomas y la vista previa de una recomendación con score y ranking de evidencia, mientras que el espacio profesional muestra la cola de casos escalados y la evidencia asociada.
...
SQL + índice reproducible & Solo relacional, NoSQL + vectorial & Separa datos transaccionales de evidencia semántica sin exigir infraestructura vectorial temprana. & Simplifica la evolución del MVP hacia nube y conserva la ruta futura a \texttt{pgvector}. \\
...
El modelo de datos del MVP se centra en entidades de usuario, consulta, síntomas, triage, recomendación, evidencia, escalamiento y trazabilidad RAG. En conjunto, estas entidades permiten representar captura de síntomas, procesamiento, recomendación, recuperación documental y transferencia del caso hacia el profesional.

Los estados de la consulta quedan definidos como submitted, recommended, escalated y reviewed. Esta secuencia no solo describe un flujo técnico; también ayuda a estructurar la trazabilidad del proceso y a preparar futuras mediciones de tiempos, volumen de escalamiento y seguimiento profesional.

\paperdiagram
  {07-data-model.png}
  {Modelo de datos del MVP con las entidades necesarias para trazabilidad clínica, recomendación fundamentada y escalamiento profesional.}
  {fig:data_model}
...
El comportamiento esperado del MVP es el siguiente: el paciente registra síntomas y contexto basal, el backend normaliza el caso clínico, el módulo de triage asigna una severidad preliminar, el módulo RAG recupera evidencia relevante del corpus curado, el sistema genera una recomendación fundamentada y, cuando corresponde, expone el caso al profesional con la información necesaria para su revisión. La Figura \ref{fig:sequence_triage} sintetiza este encadenamiento y evita que el lector vea los componentes como una lista desconectada.

\paperdiagram
  {04-sequence-triage.png}
  {Secuencia principal del MVP desde el registro de síntomas hasta la recomendación con evidencia y decisión de severidad.}
  {fig:sequence_triage}

La derivación hacia revisión humana es un flujo complementario y no una nota marginal de implementación. La Figura \ref{fig:sequence_escalation} muestra cómo el sistema conserva el contexto clínico, la salida de triage y la evidencia recuperada al escalar un caso al panel profesional.

\paperdiagram
  {05-sequence-escalation.png}
  {Secuencia de escalamiento hacia el profesional cuando la severidad o la incertidumbre superan el umbral definido.}
  {fig:sequence_escalation}
...
\midrule
Trazabilidad & La recomendacion conserva fuentes, score, ranking, metodo, fecha, severidad, decision y version de retrieval. & Es el atributo mejor representado en el MVP y el principal diferenciador frente a respuestas no fundamentadas. \\
...
Usabilidad operativa & El frontend diferencia el espacio del paciente y el del profesional segun la tarea. & La interfaz facilita comunicar el flujo y la demo ya corre sobre Compose, pero todavia requiere validacion con usuarios. \\
...
Esta entrega no pretende demostrar desempeño clínico real, ni sustituir juicio médico, ni desplegar la solución en un entorno productivo de nube. Su finalidad es fijar una arquitectura defendible, coherente con el estado del arte y acompañada de una demo reproducible con empaquetado local, documentación operativa y blueprint base para evolución posterior. En consecuencia, los resultados reportados en la sección anterior deben leerse como validación funcional inicial y no como evaluación experimental definitiva.

\subsection{Trabajo futuro}

El trabajo futuro debe concentrarse en vectorización productiva, validación clínica con expertos, endurecimiento de seguridad, despliegue real en AWS y evaluación comparativa entre embeddings locales y proveedores externos.
